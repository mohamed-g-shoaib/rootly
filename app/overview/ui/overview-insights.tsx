...
export default async function OverviewInsights({
  nowIso,
}: {
  nowIso: string
}) {
  ...
  const [entriesData, notesForTrend] = await Promise.all([
    insightsPerf.measure("entry-rows", () => getOverviewEntryRows(nowIso), (result) => ({
      rows: result.length,
    })),
    insightsPerf.measure("trend-rows", () => getOverviewTrendRows(nowIso), (result) => ({
      rows: result.length,
    })),
  ])
