...
function SurfaceCaption({ children }: { children: React.ReactNode }) {
  return (
    <div className="line-clamp-4 rounded-xl border bg-muted/35 px-3 py-2 text-sm text-pretty text-muted-foreground">
      {children}
    </div>
  )
}
      ...
      footer={
        <>
          <Badge variant="info" className="justify-center gap-2">
            <HugeiconsIcon icon={Progress03Icon} size={16} />
            Getting It
          </Badge>
          <Badge variant="outline" className="justify-center gap-2">
...
function HowItWorksCarousel({
  ...
  return (
    <div
      ref={scrollerRef}
      className="flex snap-x snap-mandatory items-start gap-4 overflow-x-auto overflow-y-hidden overscroll-x-contain pe-16 pb-2 [scrollbar-width:none] sm:pe-24 [&::-webkit-scrollbar]:hidden"
