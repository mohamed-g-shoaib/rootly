export function createSelectController({
  ...
  function getSelectOption(key, value = getSelectValue(key)) {
    return getSelectOptions(key).find((option) => option.value === value) ?? null
  }
  ...
  function focusFirstSelectOption(key) {
    const firstOption = selectRefs[key].list.querySelector("button.select-option")
    if (firstOption) {
      firstOption.focus()
    }
  }
  ...
  function moveSelectOptionFocus(key, currentButton, direction) {
    ...
    const nextButton = options[currentIndex + direction]
