...
const dynamicIconMap: Record<string, Icon> = {
  trending_up: TrendUp,
  receipt: Receipt,
};
...
export function AIAssistantAnimation({
    ...
    for (const line of lines) {
      ...
      if (line.trim() === "") {
        ...
        elements.push(
          <p
            key={key}
            className="text-wrap-pretty text-[11px] md:text-[12px] leading-[15px] md:leading-[18px] text-foreground mb-1 md:mb-1.5"
          >
            {line}
          </p>,
        );
          ...
          if (wordIndex < words.length) {
            ...
            if (segment.toolCall) {
              ...
              schedule(() => {
                ...
                segmentIndex++;
                schedule(processNextSegment, 200);
  ...
  useEffect(() => {
    if (
      ...
      setShowCards(true);
      const cardTimers: Array<ReturnType<typeof setTimeout>> = [];
                      ...
                      <div className="grid grid-cols-2 gap-2 md:gap-3">
                        {cards.map((card, cardIndex) => (
                          ...
                          >
                            ...
                            <div className="text-[12px] md:text-[14px] font-sans text-foreground">
                              {card.value}
                            </div>
        ...
        <div className="flex items-end justify-between px-2 md:px-3 pb-1.5 md:pb-2">
          <div className="flex items-center gap-1 md:gap-1.5">
            ...
            <button
              ...
              className="w-4 h-4 md:w-5 md:h-5 flex items-center justify-center transition-colors text-muted-foreground"
            >
              <Lightning size={12} className="md:w-[14px] md:h-[14px]" />
            </button>
            <button
              type="button"
              className="w-4 h-4 md:w-5 md:h-5 flex items-center justify-center transition-colors text-muted-foreground"
          ...
          <div className="flex items-end gap-1 md:gap-1.5">
            <button
              ...
              className="w-4 h-4 md:w-5 md:h-5 flex items-center justify-center transition-colors text-muted-foreground"
            >
              <Microphone size={12} className="md:w-[14px] md:h-[14px]" />
            </button>
            <button
              type="button"
