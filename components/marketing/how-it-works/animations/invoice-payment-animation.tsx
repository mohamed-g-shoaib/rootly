...
export function InvoicePaymentAnimation({
  ...
  useEffect(() => {
    ...
    const scoreBarTimers: Array<ReturnType<typeof setTimeout>> = [];
    ...
    const overdueIndices = new Set([2, 7]);
    const scheduledIndices = new Set([1]);
    const recurringIndices = new Set([4]);
    ...
    initialInvoices.forEach((invoice, index) => {
      const timer = setTimeout(
        () => {
          const isOverdue = overdueIndices.has(index);
          ...
          if (index === invoiceCount - 1) {
            setPaidCount((prev) => prev + paidInvoiceCount);
            setPaidAmount((prev) => {
    ...
    return () => {
      ...
      scoreBarTimers.forEach(clearTimeout);
      ...
      <div className="flex-1 relative overflow-hidden flex flex-col z-10">
        ...
        {showCards && (
          <div className="grid grid-cols-2 gap-3 md:gap-4 pt-4 md:pt-6 pb-4 md:pb-6">
            ...
            >
              <div className="font-sans text-base md:text-lg text-foreground mb-1 md:mb-1.5">
                {openAmount}
              </div>
            ...
            >
              <div className="flex items-center justify-between mb-1.5 md:mb-2">
                <div className="font-sans text-base md:text-lg text-foreground">
                  Good
                </div>
                {showPaymentScore && (
