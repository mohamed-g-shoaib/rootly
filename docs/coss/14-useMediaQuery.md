...
A React hook that subscribes to a CSS media query and returns whether it matches. Built on `useSyncExternalStore` for SSR safety and concurrent mode compatibility. It's for responsive logic in React: the hook checks whether a CSS media query matches and gives you a boolean like true or false, then updates when the viewport or device state changes. In practice, that means your component can react to screen size, orientation, dark mode preference, or reduced-motion preference in JavaScript, not just in CSS.

## When to use it

Use it when the UI behavior itself should change, not only the styling. For example, you might render a mobile drawer on small screens and a permanent sidebar on desktop, or disable animations when the user prefers reduced motion.
