...
[Enrollment for my animation course is open!](http://animations.dev/) Enrollment has closed.
...
Hover over one of the icons to see a tooltip, then quickly hover over another icon within the same group.
...
You could decrease the duration to make `ease-in` work, but this easing is just not made for UI animations. It speeds up at the _end_, which is the opposite of what we want.
...
You can still apply the easing principles, just use a custom `ease-out` for more impact. There are plenty of sites that offer custom variations of all easings, here’s one I recommend: [easings.co](https://easings.co/).
...
Click on the feedback button below to open the popover and see it animate from the center, close it, and press `J` tap on the grey X icon to set the correct origin and open the popover again.  
...
I go into this topic in more detail in the ["You Don’t Need Animations"](/ui/you-dont-need-animations) post.
...
In [animations.dev](https://animations.dev) we go way deeper into the theory and practice of great animations. There are videos and interactive components that help you understand the theory, and exercises for you to practice building animations that feel right.
